# Hướng dẫn sử dụng Odama với Laravel Octane

Laravel Octane tăng tốc ứng dụng của bạn bằng cách giữ ứng dụng trong bộ nhớ giữa các request. Tuy nhiên, điều này có thể dẫn đến các vấn đề về bảo mật và rò rỉ dữ liệu nếu không được cấu hình đúng cách. Tài liệu này cung cấp hướng dẫn chi tiết để đảm bảo Odama hoạt động tốt và bảo mật với Laravel Octane.

## Cài đặt

Thư viện Odama đã được thiết kế để tương thích với Laravel Octane. Khi phát hiện Laravel Octane, `OdamaServiceProvider` sẽ tự động đăng ký `OctaneServiceProvider` để xử lý các vấn đề tương thích.
...
```php
use Odama\Concerns\OctaneCompatible;
...
Nếu bạn gặp vấn đề khi sử dụng Odama với Laravel Octane, vui lòng tạo issue trên GitHub hoặc liên hệ support@odama.dev. 